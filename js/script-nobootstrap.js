...
let pokemonRepository = (function () {
  ...
  //   
  //     $('#pokemonModal').modal("show"); // Bootstrap’s own function to make the modal appear
  //   });

  /// creating each pokemon button
  function addListItem(pokemon) {
    //let pokemonList = $("btn btn-primary");
    let listPokemon = $('<li class="group-list-item"></li>');
    let button = $(
      '<button type= "button" class="pokemon-button btn" data-target= "#pokemonModal" datatoggle="modal">' +
        pokemon.name +
        "</button>"
    );
